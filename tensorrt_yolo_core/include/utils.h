...
inline void yolo_detece2detect_result(tensorrt_yolo_msg::msg::Results& results_msg_, std::vector<detect_result>& objects){
    for(int i = 0; i < results_msg_.results.size();i++){
        detect_result det;
        det.classId = results_msg_.results[i].class_id;
...
// 绘制追踪边框
inline void draw_tracking_id(cv::Mat& img, std::vector<strack>& output_stracks, tensorrt_yolo_msg::msg::Results& results_msg_){
    for (unsigned long i = 0; i < output_stracks.size(); i++)
    {
        tensorrt_yolo_msg::msg::InferResult inf;
        for(int j = 0;j < 4; j ++){
            inf.bbox[j] = output_stracks[i].tlbr[j];
        }
        inf.conf = output_stracks[i].score;
        inf.class_id = output_stracks[i].classId_;
        ...
        {
            cv::Scalar s((inf.class_id * 30 + 123) % 255, (inf.class_id * 20 + 78) % 255 , (inf.class_id + 478) % 255); // 随机颜色
...
// 绘制目标检测边框
inline void draw_detection_box(cv::Mat& img, tensorrt_yolo_msg::msg::Results& results_msgs_){
    for(tensorrt_yolo_msg::msg::InferResult inferResult : results_msgs_.results) {
        // 在图像上绘制检测结果

        cv::Scalar bboxColor((inferResult.class_id * 30 + 123) % 255, (inferResult.class_id * 20 + 78) % 255,
                             (inferResult.class_id + 478) % 255); // 随机颜色
        ...
        std::string className = vClassNames[(int) inferResult.class_id]; // 获取类别名称
...
// 计算并绘制相机坐标中心点坐标
inline void calculate_draw_center_point(cv::Mat& img, cv::Mat& depth_img, tensorrt_yolo_msg::msg::Results& results_msg_){
    std::stringstream stream;
    for(int i = 0;i<results_msg_.results.size();i++) {
        cv::Scalar bboxColor((results_msg_.results[i].class_id * 30 + 123) % 255, (results_msg_.results[i].class_id * 20 + 78) % 255,
                             (results_msg_.results[i].class_id + 478) % 255); // 随机颜色
        ...
        {
            RCUTILS_LOG_WARN("Pixel coordinates not set. Skipping depth image processing.");
            return;
        }
        ...
        try
        {
            ...
            {
                RCUTILS_LOG_WARN("Pixel coordinates out of bounds.");
                return;
            }
...
// 计算并绘制像素坐标中心点
inline void calculate_draw_center_point(cv::Mat& img, tensorrt_yolo_msg::msg::Results& results_msg_) {
    for(int i = 0;i<results_msg_.results.size();i++) {
        cv::Scalar bboxColor((results_msg_.results[i].class_id * 30 + 123) % 255, (results_msg_.results[i].class_id * 20 + 78) % 255,
                             (results_msg_.results[i].class_id + 478) % 255); // 随机颜色
        int pixel_x = round((results_msg_.results[i].bbox[0] + results_msg_.results[i].bbox[2]) / 2.0);
...
inline void draw_pose_points(cv::Mat& img, tensorrt_yolo_msg::msg::Results results_msg_){
    ...
    for (size_t j = 0; j < results_msg_.results.size(); j++)
    {
        ...
        std::vector<tensorrt_yolo_msg::msg::KeyPoint> vScaledKpts = results_msg_.results[j].vkpts;
        ...
        cv::Scalar skeletonColor((results_msg_.results[j].class_id * 30 + 123) % 255, (results_msg_.results[j].class_id * 20 + 78) % 255,
                             (results_msg_.results[j].class_id + 478) % 255); // 随机颜色
