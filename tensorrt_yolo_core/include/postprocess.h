...
__inline__ std::vector<tensorrt_yolo_msg::msg::KeyPoint> scale_kpt_coords(cv::Mat& img, float* pkpt, int kNumKpt, int kKptDims){
    float r_w = kInputW / (img.cols * 1.0);
    float r_h = kInputH / (img.rows * 1.0);
    float r = std::min(r_w, r_h);
    float pad_h = (kInputH - r * img.rows) / 2;
    ...
    std::vector<tensorrt_yolo_msg::msg::KeyPoint> vScaledKpts;
    float x;
    float y;
    float conf;
    float* pSingleKpt;
    for (int i = 0; i < kNumKpt; i++){
        ...
        y = y / r;
        conf = pSingleKpt[2];
        tensorrt_yolo_msg::msg::KeyPoint scaledKpt;
        scaledKpt.x = x;
        scaledKpt.y = y;
