...
ImageInfer::ImageInfer(rclcpp::Node::SharedPtr& node):
save_(true),
imageDir_(imageDir),
YoloDetector(node)
{
...
int ImageInfer::run(){
    ...
    for (long unsigned int i = 0; i < file_names_.size(); i++){
        ...
        tensorrt_yolo_msg::msg::Results res = inference(img);
...
void ImageInfer::draw_image(cv::Mat& img, tensorrt_yolo_msg::msg::InferResult inferResult){
    // 在图像上绘制检测结果

    cv::Scalar bboxColor((inferResult.class_id * 30 + 123) % 255, (inferResult.class_id * 20 + 78) % 255 , (inferResult.class_id + 478) % 255); // 随机颜色
...
int main(int argc, char *argv[])
{
    ...
    auto node = std::make_shared<rclcpp::Node>("image_infer_node");
    ImageInfer i(node);
    rclcpp::spin(node);
    rclcpp::shutdown;
    return 0;
